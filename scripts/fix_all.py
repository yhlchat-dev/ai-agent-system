...
# ÐÞ¸´ 1: sensitive_detector.py 
sensitive_content = '''#!/usr/bin/env python 
# -*- coding: utf-8 -*- 
import re 
...
def scan_text(text: str) -
    ...
    api_pattern = r"(?:api[_-]?key|sk-)[a-zA-Z0-9]{16,}" 
    if re.search(api_pattern, text, re.IGNORECASE): 
        sensitive_types.append("api_key") 
        masked_text = re.sub(api_pattern, "****", masked_text) 
    return DetectionResult(is_sensitive=len(sensitive_types), sensitive_types=sensitive_types, masked_text=masked_text, original_text=text) 
''' 
with open("infra/sensitive_detector.py", "w", encoding="utf-8") as f: 
    f.write(sensitive_content) 
print("? sensitive_detector.py ÒÑÐÞ¸´") 
